...
import { X, ShieldCheck, Target, Video, Bell, AlertTriangle, Zap, Wind, Box, LifeBuoy, Sun, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const TacticalHUD: React.FC<TacticalHUDProps> = ({
  ...
  const pages = [
    ...
    { 
      id: 'sailsteer', 
      title: 'SAILSTEER', 
      component: <SailSteerPage sog={sog} hdg={hdg} twd={twd} tws={tws} twa={twa} /> 
    },
    { 
      id: 'nav3d', 
      title: 'NAVEGACIÓN', 
      component: <Navigation3DPage btw={navData.btw} dtw={navData.dtw} xte={navData.xte} hdg={hdg} waypointName={navData.waypointName} shipId={shipId} /> 
    },
    { 
      id: 'laylines', 
      title: 'LAYLINES', 
    ...
    { 
      id: 'depth', 
      title: 'PROFUNDIDAD', 
      component: <DepthPage currentDepth={depth} depthHistory={depthHistory} shipId={shipId} /> 
    },
    { 
      id: 'basic_sd', 
    ...
    { 
      ...
      component: <PlotPage 
        mode="simple" 
        data={[{ label: 'TEMP AGUA', value: '18.4', unit: '°C', color: '#10b981' }]} 
      /> 
    },
    { 
      id: 'plot_dual', 
      ...
      component: <PlotPage 
        mode="dual" 
        data={[
          { label: 'BSPD', value: sog.toFixed(1), unit: 'KT', color: '#3b82f6' },
          { label: 'TWS', value: tws.toFixed(1), unit: 'KT', color: '#f59e0b' }
        ]} 
      /> 
    },
  ];
          ...
          >
            ...
            <div className="bg-black px-4 pt-3 pb-1 flex items-center justify-between z-40 border-b border-white/5">
                <div className="flex items-center gap-2">
                    <div className={cn("w-1.5 h-1.5 rounded-full shadow-lg transition-all duration-1000", isNavigating ? "bg-[#00ffcc] animate-pulse" : "bg-slate-800")} />
                    <span className="text-[7px] font-black text-slate-500 uppercase tracking-[0.3em] font-sans">{pages[initialPageIndex].title}</span>
                </div>
                ...
                {isPagesMenuOpen && (
                  ...
                  >
                    <div className="px-5 mb-4">
                        <span className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.3em]">SELECT PAGE</span>
                    </div>
                    <div className="flex-grow overflow-y-auto px-2 space-y-1 custom-scrollbar">
                        {pages.map((p, idx) => (
                          ...
                          >
                            <span className="w-4 text-center opacity-30">{idx + 1}</span>
                            {p.title}
                          </button>
                        ))}
                    </div>
            ...
            <div className="bg-[#121212] px-3 pt-5 pb-9 flex items-center justify-center gap-2.5 border-t border-black relative">
               ...
               <div className="absolute inset-0 opacity-[0.04] pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle, #fff 1px, transparent 1px)', backgroundSize: '3px 3px' }} />
               ...
               >
                  <span className="text-[7px] font-black text-slate-500 group-hover:text-white transition-colors tracking-tight">PAGES</span>
                  <div className="mt-1 flex flex-col gap-0.5 opacity-60 group-hover:opacity-100">
                    <div className="flex gap-0.5"><div className="w-1.5 h-1.5 bg-slate-400 rounded-sm"/><div className="w-1.5 h-1.5 bg-slate-400 rounded-sm"/></div>
                    <div className="flex gap-0.5"><div className="w-1.5 h-1.5 bg-slate-400 rounded-sm"/><div className="w-1.5 h-1.5 bg-slate-400 rounded-sm"/></div>
                  </div>
               </button>
               ...
               <button 
                  className="flex flex-col items-center justify-center w-[62px] h-[44px] bg-[#1a1a1a] rounded-lg border-t border-white/10 border-b-[5px] border-black active:translate-y-[2px] active:border-b-[2px] transition-all shadow-[0_6px_12px_rgba(0,0,0,0.6)] group z-10"
               >
                  <span className="text-[7px] font-black text-slate-500 group-hover:text-white transition-colors tracking-tight">MENU</span>
                  <Sun size={12} className="mt-1 text-slate-600 group-hover:text-amber-400 transition-colors" />
               </button>
            </div>
